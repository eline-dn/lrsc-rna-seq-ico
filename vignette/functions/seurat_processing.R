# the purpose of this script is to process the count matrices, store the expression data in a Seurat object and QC it 
...
number_of_isoforms_per_gene <- function(seu_obj,sample_name,
  ...
  axis.title.y = element_text(size = 20),   # titre axe Y
  axis.text.x  = element_text(size = 16),   # texte graduations X
  axis.text.y  = element_text(size = 16),   # texte graduations Y
  plot.title   = element_text(size = 18, hjust = 0.5) 
          )
  ggsave(filename=paste0(outdir,"/",sample_name,"_number_iso_gene.png"),plot=plot)
  print(plot)
  return(plot)
}
...
top10_gene_with_most_iso<- function(seu_obj,sample_name,
  ...
  write.csv(gene_transcript_counts, paste0(outdir,"/",sample_name,"_gene_most_iso.csv"))
  ...
  ggsave(filename=paste0(outdir,"/",sample_name,"_gene_most_iso.png"),plot=plot)
...
# to regress on cell cycle and on ribosomal genes  
...
###########
# to regress on cell cycle genes only
...
run_regression_and_merge_v2 <- function(sample_names, # a list of sample-names in the right order
  ...
  dir.create(paste0(output_path), recursive = TRUE, showWarnings = FALSE)
  ...
  merged_obj<-PrepSCTFindMarkers(merged_obj, assay = "SCT", verbose = TRUE)
  saveRDS(merged_obj, file = paste0(output_path, "/regressed_obj_", type, integ_suffix, ".rds"))
  message(integ_suffix," object saved in:",output_path, "/regressed_obj_", type, integ_suffix, ".rds")
  ...
  ggsave(paste0(output_path, "/dimplot_phase_after_reg", type, integ_suffix, ".jpg"),
         plot = plot_phase, height = 7, width = 12)
  ...
  if (integrateRPCA) {
    ...
    ggsave(paste0(output_path, "/merged_integrated_", type,"_dimplot.png"), plot = dimplot)
    #print(dimplot)
  } else {
    dplot <- DimPlot(merged_obj, group.by = c("clusters", "sample")) +
      labs(title = paste("UMAP colored by clusters",integ_suffix, type))
    ggsave(paste0(output_path, "/dimplot_clusters_", type,integ_suffix,".png"), plot = dplot)
    #print(dplot)
  ...
  dplot <- DimPlot(merged_obj, group.by = "sample") +
    labs(title = paste("UMAP colored by sample",integ_suffix,type))
  ggsave(paste0(output_path, "/dimplot_sample_", type,integ_suffix,".png"), plot = dplot)
  #print(dplot)
...
# a function to plot and save venn diagramms to visualise shared genes and isoforms between samples in a seurat object. 
